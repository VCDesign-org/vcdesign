...
> 各レベルの canonical 定義は **[maturity-levels.md](maturity-levels.md)** を参照。
> 本ファイルは層別ルーブリック（evidence / failure signals / blocking / regression）を定義する。
...
- IDG が B1・B4 境界に実装されており、bypass ルートが存在しないことがコードまたはログで確認できる
- AI 出力ログに `judgment_proposal: true` フラグが含まれている（サンプル確認可）
- Judgment Closure 記録に `decided_by`（人間ロール）・`decided_at`・`evidence_reviewed` フィールドが存在する
...
Evidence required (verifiable):
- Agent 実行ログに policy チェック通過の記録が存在し、チェック内容（境界・スコープ・承認者）が含まれている
- Agent 起動設定ファイルまたはデプロイ記録に `halt_owner` フィールドが named・staffed 状態で存在する
- 不可逆操作のログに `approver_identity`・`approved_at`・`reversibility_assessed: true` が含まれている
...
Evidence required (verifiable):
- 直近の判断ログに `decided_by`（人間の名前またはロール）・`decided_at`・`decision_reason` が含まれている（サンプル5件以上確認可）
- halt owner が各 Agent・操作のデプロイ記録に named かつ現在 staffed であることが確認できる
- エスカレーション経路の使用記録が過去1レビューサイクル内に1件以上存在する（または drill 記録が存在する）
...
Evidence required (verifiable):
- Fast loop から slow loop 制約への書き込みパスにゲートが実装されており、バイパス不可であることがコードまたはアーキテクチャ図で確認できる
- Resolution レコードに `expires_at` フィールドが存在し、期限切れ時にアクティブとして扱われないことが確認できる
- 価値目標の変更コミットまたは設定変更ログに Value Loop 審査者の承認が紐づいている
...
Evidence required (verifiable):
- halt drill の実施記録（日時・参加者・結果）が過去1レビューサイクル内に存在する
- 部分 Agent 障害をシミュレートした halt テストの成功記録が存在する
- Override 後の安全復帰手順がドキュメント化されており、実施者が訓練済みであることが記録されている
- `halt_readiness` メトリクスが直近の観測サイクルで閾値内にある
...
## Regression Signals（層別）

成熟度は上がるだけでなく下がる。以下のシグナルが観測された場合、その層が一段階退行している可能性がある。
退行は失敗ではなく通常の劣化として扱い、再診断と是正アクションに繋げる。

### Model Layer の退行シグナル
- AI 推薦の自動承認率が上昇し始めている（Enforced → Defined）
- 新しいモデルや機能追加時に IDG ゲートのレビューがスキップされている
- Judgment Proposal のログ品質が低下している（`decided_by` が空・`evidence_reviewed` が省略）

### Agent Layer の退行シグナル
- 承認なしの本番操作が「例外的に」増えている（Enforced → Defined）
- halt_owner のポジションが空席になっているが Agent が継続稼働している
- Agent のスコープ宣言が更新されないまま実際の操作範囲が拡大している
- policy チェックログが「承認済み」で埋まっているが内容が形骸化している

### Responsibility Layer の退行シグナル
- 承認が Slack DM や口頭に移行し、ログが残っていない（Enforced → Defined）
- `owner_missing` メトリクスが再び発火し始めている
- 期限切れの Defer が再びレビューなしで積み上がっている
- 責任者が「担当チーム全体」という表現に戻っている

### Time Layer の退行シグナル
- `expires_at` を設定しない Resolution が再び増えている（Enforced → Defined）
- 期限切れ判断が「まだ有効」として使われているケースがある
- fast loop の変更が slow loop のレビューなしに value 目標を暗黙的に書き換えている
- 季節・環境変動への対応が「運用判断」として記録されず素通りしている

### Human Sovereignty Layer の退行シグナル
- halt drill が直近のレビューサイクルで実施されていない（Enforced → Defined）
- override 手順を知っている担当者が離職・異動し、引き継ぎが未完了
- 「止め方は AI が止まれば OK」という認識が現場に広まっている
- high_impact 操作の外部責任説明（採用理由・責任経路）が省略され始めている
