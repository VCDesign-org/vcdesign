...
### 位置づけ：設計記法ではなく運用規律（2026-09 改訂）

VC-AD／TDDD が担う記法層は、AI駆動開発のループ設計が標準化していくにつれて、
その共通言語として吸収されていく可能性が高い領域です。VCDesignの軸足は、
記法そのものではなく、**責任の保持者に張り付いて離れない運用規律**——
決定と同じ粒度で「決めないと決めたこと」を記録し、境界の一個外までを
検証範囲に含める、といった規律——に置かれます。ここは検証者を機械に
委ねられないため、標準化に吸収されにくい領域です。

規律は文脈を共有していないと伝わらず、記法のように誰でも試せるわけでは
ありません。そのため記法層は廃止せず、**規律への入口**として残します。
まず VC-AD / TDDD から手を動かし、そこから運用規律に接続してください。
